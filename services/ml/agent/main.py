...
async def entrypoint(ctx: JobContext):
    ...
    # Определяем создателя (первый подключившийся не-агент)
    creator = None
    for p in ctx.room.participants.values():
        if p.identity != ctx.local_participant.identity:
            creator = p
            break
    if creator is None:
        logger.info("⏳ Waiting for first participant (room creator)...")
        creator = await ctx.wait_for_participant()
    logger.info(f"👤 Creator identified: {creator.identity}")

    # Синхронный обработчик для случая, когда аудиотрек появляется позже
    ...
    # Проверяем, есть ли уже аудиотрек создателя
    audio_track = None
    for pub in ctx.room.participants.get(creator.sid).tracks.values():
        if pub.track and pub.track.kind == "audio":
            audio_track = pub.track
            break

    if audio_track:
        logger.info(f"🎤 Audio track already available, starting immediately...")
        asyncio.create_task(process_audio_track(audio_track))
    else:
        logger.info(f"⏳ Waiting for creator's audio track...")
