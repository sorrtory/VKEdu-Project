...
- В `infra/env/prod.env` и `infra/env/prod.override.env` заданы production
  значения. Deploy после `git-crypt unlock` копирует их в корневые `.env` и
  `.env.production`.
...
cp infra/env/prod.env .env
cp infra/env/prod.override.env .env.production

# отредактировать оба файла или исходные infra/env/* по примерам ниже
${EDITOR:-nano} .env
...
MLIN_KAFKA_TOPICS=conference.chat,conference.chat.ai.request,conference.chat.ai.response,conference.chat.file,conference.boardcrop,conference.transcript.voice
MLOUT_KAFKA_GROUP_ID=mlout_group
MLOUT_KAFKA_TOPICS=conference.chat.ai.request,conference.summary.request
LLM_API_KEY=change-me-to-your-llm-api-key
LLM_BASE_URL=https://api.vsellm.ru/v1
LLM_MODEL=yandex/gpt5-lite
...
Основной workflow находится в `.github/workflows/deploy-published-prod.yml`. После успешного `.github/workflows/build-publish-containers.yml` на ветке `deploy` он заходит на сервер по SSH, обновляет ветку `deploy`, разблокирует `git-crypt`, копирует расшифрованные `infra/env/prod.env` и `infra/env/prod.override.env` в корневые `.env` и `.env.production`, затем запускает Docker Compose с `--pull always`, чтобы подтянуть опубликованные образы из GHCR.
